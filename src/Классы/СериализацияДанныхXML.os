...
// Выполняет сериализацию данных в файл в соответствии с заданными параметрами
//
// Параметры:
//   ДанныеЗаписиXML - Соответствие, Массив, Структура, Число, Строка, Дата - данные для сериализации в XML
//   ПараметрыЗаписи - См. НовыеПараметрыЗаписиXML
//
Процедура ЗаписатьВФайлСПараметрами(ДанныеЗаписиXML, ПараметрыЗаписи) Экспорт

	ЗаписьXML = Новый ЗаписьXML;

	Если ТипЗнч(ПараметрыЗаписи.ПараметрыЗаписиXML) = Тип("ПараметрыЗаписиXML") Тогда
		ЗаписьXML.ОткрытьФайл(
			ПараметрыЗаписи.ПутьКФайлу,
			ПараметрыЗаписи.ПараметрыЗаписиXML,
			ПараметрыЗаписи.ДобавлятьBOM);
	Иначе
		ЗаписьXML.ОткрытьФайл(ПараметрыЗаписи.ПутьКФайлу, , ПараметрыЗаписи.ДобавлятьBOM);
	КонецЕсли;

	Если ПараметрыЗаписи.ЗаписатьОбъявлениеXML Тогда
		ЗаписьXML.ЗаписатьОбъявлениеXML();
	КонецЕсли;
...
// Возвращает новые параметры записи файла XML
//
// Возвращаемое значение:
//	 Структура:
//    * ПутьКФайлу - Строка - путь к файлу с данными xml
//    * ЗаписатьОбъявлениеXML - Булево - признак добавления записи объявления XML
//    * ПараметрыЗаписиXML - ПараметрыЗаписиXML - Определяет набор параметров, используемых при записи XML
//    * ДобавлятьBOM - Булево - Определяет, будет ли добавлена метка порядка байт (BOM) к результирующему файлу XML
//
Функция НовыеПараметрыЗаписиXML() Экспорт
	
	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("ПутьКФайлу", "");
	ПараметрыЗаписи.Вставить("ЗаписатьОбъявлениеXML", Ложь);
	ПараметрыЗаписи.Вставить("ПараметрыЗаписиXML", Неопределено);
	ПараметрыЗаписи.Вставить("ДобавлятьBOM", Истина);
